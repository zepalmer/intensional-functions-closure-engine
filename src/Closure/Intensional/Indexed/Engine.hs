...
-- TODO: if we are conceding that computations should be orderable, then the
-- addComputation function should not pull on the computation immediately.
-- Instead, computations should be added to the workset and introduced to the
-- engine at a later time.  This would prevent e.g. two identical monadic
-- effects from being produced if two identical computations were added.
...
data EngineSuspensionFunctor (m :: Type -> Type) fact result
  = ESFAddComputation (Computation m fact) (() ->%Ord result)
  | ESFAddFacts (Set fact) (() ->%Ord result)
  | ESFAwaitIndex (ComputationAwaitingIndex fact result)

deriving instance
    ( Eq fact
    , Eq (m (CoroutineStepResult Ord
                (EngineSuspensionFunctor m fact) m (Set fact)))
    ) => Eq (EngineSuspensionFunctor m fact result)
deriving instance
    ( Ord fact
    , Ord (m (CoroutineStepResult Ord
                (EngineSuspensionFunctor m fact) m (Set fact)))
    ) => Ord (EngineSuspensionFunctor m fact result)
...
instance IntensionalFunctor (EngineSuspensionFunctor m fact) where
    ...
    itsFmap = \%%Ord fn functor ->
      case functor of
        ESFAddComputation computation contfn ->
          ESFAddComputation computation (itsCompose %@% (fn, contfn))
...
pullOnComputation computation engine =
  intensional Ord do
    ...
    case result of
      Left (ESFAddComputation computation' contfn) ->
        intensional Ord do
          (engine', newFacts) <-
      ...
      Right factSet ->
        ...
        itsPure %@ (Nothing, addFacts (Set.toList newFacts) engine, newFacts)
...
close engine =
  if isClosed engine then itsPure %@ engine else
    intensional Ord do
      engine' <- step engine
      close engine'
...
addIntermediateComputation computation =
    suspend (ESFAddComputation computation itsPure)
